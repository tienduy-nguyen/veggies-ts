...
describe('extensions > fixtures > index', () => {
    ...
    describe('load', () => {
        const featureDir = 'tests/unit/extensions/fixtures/__mocks__'
        ...
        it('generic load of .txt fixture file', async () => {
            loadTextStub.returns(textContent)
            await fixtures.load('text_file').then((data) => {
                expect(data).toEqual(textContent)
                expect(loadTextStub.calledWithExactly(`${fixturesDir}/text_file.txt`)).toBeTruthy()
            })
        })

        it('generic load of .js fixture file', async () => {
            loadModuleStub.returns(jsContent)
            await fixtures.load('js_file').then((data) => {
                expect(data).toEqual(jsContent)
                expect(loadModuleStub.calledWithExactly(`${fixturesDir}/js_file.js`)).toBeTruthy()
            })
        })

        it('generic load of .json fixture file', async () => {
            loadJsonStub.returns(jsonContent)
            await fixtures.load('json_file').then((data) => {
                expect(data).toEqual(jsonContent)
                expect(loadJsonStub.calledWithExactly(`${fixturesDir}/json_file.json`)).toBeTruthy()
            })
        })

        it('generic load with multiple matching fixture files', async () => {
            await fixtures.load('multi').catch((err: { message: string }) => {
                expect(err.message).toEqual(
                    [
                        `Found 2 matching fixture files, you should have only one matching 'multi', matches:`,
                        '  - tests/unit/extensions/fixtures/__mocks__/fixtures/multi.json',
                        '  - tests/unit/extensions/fixtures/__mocks__/fixtures/multi.yaml',
                    ].join('\n')
                )
            })
        })

        it('generic load without feature uri', async () => {
            fixtures.setFeatureUri(undefined)

            await fixtures.load('fixture').catch((err: { message: string }) => {
                expect(err.message).toEqual('Cannot load fixture: fixture, no feature uri defined')
            })
        })

        it('generic load with no matching fixture file', async () => {
    ...
    describe('loadModule', () => {
        ...
        it('load valid .js fixture file', async () => {
            relativePathStub.returns(
                `'../../../tests/unit/extensions/fixtures/__mocks__/fixture.js`
            )
            await fixtures.loadModule('fixture.js').then((data) => {
                expect(data).toEqual(jsContent)
            })
        })

        it('load non existing .js fixture file', async () => {
            relativePathStub.returns('../../../tests/unit/extensions/fixtures/__mocks__/noent.js')
...
error: Cannot find module '../../../tests/unit/extensions/fixtures/__mocks__/noent.js' from 'src/extensions/fixtures/index.ts'`
        ...
        it('load .js without default exported function', async () => {
            relativePathStub.returns(
                '../../../tests/unit/extensions/fixtures/__mocks__/no_default_function.js'
            )
            await fixtures
    ...
    describe('loadJson', () => {
        ...
        it('load non existing .json fixture file', async () => {
            loadTextStub
                .withArgs('noent.json')
        ...
        it('load invalid .json fixture file', async () => {
            loadTextStub
